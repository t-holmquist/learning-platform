...
const MobileTabNav = () => {
  ...
  return (
    ...
    <div className={`flex justify-center ${pathName == "/sign-in" && "hidden"} ${pathName == "/" && "hidden"} `}>
        <div className="sm:hidden w-11/12 flex justify-between items-center fixed bottom-4 px-6 py-3 z-20  rounded-large shadow-primary bg-bg-red border">
          {/* Dashboard */}
          <Link href={"/dashboard"}>
          ...
          {/* Design assets */}
            <svg  width="36" height="36" viewBox="0 0 133 133" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <AnimatePresence>
                {isAssetsOpen && (
                        ...
                        exit={{opacity: 0, y: 20}}
                        className="absolute flex flex-col px-4 gap-2 -top-21 -left-60 translate-x-30 border bg-secondary rounded-small">
